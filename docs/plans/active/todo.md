...
5. [ ] Define and enforce deterministic performance gates for interactive TOP viewing on RTX 2060 baseline hardware: real-time target `>=60 FPS @1080p`, idle headroom primary gate `p95 frametime <= 10 ms`, secondary gate `average FPS >= 90` in empty/light scenes.
...
7. [ ] Profile node-editor and TOP-viewer hotspots on representative heavy graphs; ship highest-impact optimizations without visual regressions.
