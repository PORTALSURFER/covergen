...
3. [ ] Execute Phase 1 of `docs/plans/active/geometry_nodes.md`: land analytic geometry primitives in strict order (`buf.box`, `buf.grid`, `buf.tube`, `buf.torus`).
4. [ ] Finish the immediate V1 node-registry delivery slice with strict typed ports and no implicit conversions.
5. [ ] Complete output-driven pull evaluation with deterministic dirty propagation and coverage for failure/boundary paths.
6. [ ] Advance Windows GPU export throughput path for H.264/image sequence workflows, sequencing NVENC first and AMF second, targeting zero-readback where feasible.
7. [ ] Keep handoff docs synchronized (`AGENTS.md`, `MEMORY.md`, `docs/plans/active/todo.md`) whenever priorities or status change.
